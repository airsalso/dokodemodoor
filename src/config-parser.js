...
/**
 * [목적] JSON Schema 및 보안 규칙 기반 설정 검증.
 *
 * [호출자]
 * - parseConfig()
 */
  ...
  if (config.authentication) {
    ...
    if (auth.credentials) {
      for (const pattern of DANGEROUS_PATTERNS) {
        ...
        if (auth.credentials.totp_code && pattern.test(auth.credentials.totp_code)) {
          throw new Error('authentication.credentials.totp_code contains potentially dangerous pattern');
        }
      }
    }
...
const sanitizeAuthentication = (auth) => {
  return {
    ...
    credentials: {
      ...
      ...(auth.credentials.totp_code && { totp_code: auth.credentials.totp_code.trim() }),
